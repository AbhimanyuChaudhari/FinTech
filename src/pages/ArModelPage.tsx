...
export default function ARForecastPage() {
  ...
  const prepareChartData = () => {
    ...
    const upperCI = forecast.ci_upper || [];
    const lowerCI = forecast.ci_lower || [];
    ...
    for (let j = 0; j < predicted.length; j++) {
      const lower = lowerCI[j];
      const upper = upperCI[j];
      data.push({
  ...
  return (
    <div style={{ backgroundColor: "#0f172a", color: "#f1f5f9", minHeight: "100vh", padding: "2rem" }}>
      <h1 style={{ fontSize: "1.8rem", color: "#60a5fa", marginBottom: "1rem" }}>
        Autoregressive Model Forecast for {ticker}
      </h1>
      ...
      <div style={{ display: "flex", gap: "1rem", marginBottom: "1rem", alignItems: "center" }}>
        ...
        <label>
          Lambda:
          <input type="number" value={lambda} step={0.01} min={0} onChange={(e) => setLambda(Number(e.target.value))} style={{ marginLeft: "0.5rem", width: "60px" }} />
        </label>
        <label>
          Max P:
          <input type="number" value={maxP} min={1} onChange={(e) => setMaxP(Number(e.target.value))} style={{ marginLeft: "0.5rem", width: "60px" }} />
        </label>
      ...
      <div style={{ marginBottom: "1rem" }}>
        <button onClick={() => setViewMode("log")} style={{ marginRight: "1rem", padding: "0.5rem 1rem", backgroundColor: viewMode === "log" ? "#38bdf8" : "#1e293b", color: "white", border: "none", borderRadius: "8px" }}>Log Return View</button>
          ...
          <div style={{ width: "100%", height: 400, background: "#1e293b", borderRadius: "12px", padding: "1rem" }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                {forecast.ci_upper && forecast.ci_lower && (
                  <>
                    <Area
                      type="monotone"
                      dataKey="ci_range"
